...
int op_stats_node::_run_server() {
	...
	case stats_type_threads:
		this->_send_stats_threads(singleton<novadbd>::instance().get_req_thread_pool(), singleton<novadbd>::instance().get_other_thread_pool());
		break;
	case stats_type_threads_request:
		this->_send_stats_threads(singleton<novadbd>::instance().get_req_thread_pool(), singleton<novadbd>::instance().get_other_thread_pool(), thread_pool::thread_type_request);
		break;
	case stats_type_threads_slave:
		{
			cluster* cl = singleton<novadbd>::instance().get_cluster();
			vector<cluster::node> slave = cl->get_slave_node();
			for (vector<cluster::node>::iterator it = slave.begin(); it != slave.end(); it++) {
				this->_send_stats_threads(singleton<novadbd>::instance().get_req_thread_pool(), singleton<novadbd>::instance().get_other_thread_pool(), it->node_thread_type);
			}
		}
		break;
	case stats_type_nodes:
		this->_send_stats_nodes(singleton<novadbd>::instance().get_cluster());
		break;
	case stats_type_threads_queue:
		this->_send_stats_threads_queue();
		break;
	default:
		this->_send_stats(singleton<novadbd>::instance().get_req_thread_pool(),singleton<novadbd>::instance().get_other_thread_pool(),
				singleton<novadbd>::instance().get_storage(),
				singleton<novadbd>::instance().get_cluster());
		break;
	}
