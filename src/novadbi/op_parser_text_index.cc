...
op* op_parser_text_index::_determine_op(const char* first, const char* buf, int& consume) {
	...
	if (strcmp(first, "ping") == 0) {
		...
		if (strcmp(second, "add") == 0) {
			r = new op_node_add(this->_connection, singleton<novadbi>::instance().get_cluster()); 
		} else if (strcmp(second, "remove") == 0) {
			r = new op_node_remove(this->_connection, singleton<novadbi>::instance().get_cluster()); 
		} else if (strcmp(second, "role") == 0) {
			r = new op_node_role(this->_connection, singleton<novadbi>::instance().get_cluster()); 
		} else if (strcmp(second, "state") == 0) {
			r = new op_node_state(this->_connection, singleton<novadbi>::instance().get_cluster()); 
		} else {
			r = new op_error(this->_connection); 
		}
	} else if (strcmp(first, "kill") == 0) {
		r = new op_kill(this->_connection, singleton<novadbi>::instance().get_req_thread_pool(), singleton<novadbi>::instance().get_other_thread_pool()); 
	} else if (strcmp(first, "quit") == 0) {
		r = new op_quit(this->_connection); 
	} else if (strcmp(first, "meta") == 0) {
		r = new op_meta(this->_connection, singleton<novadbi>::instance().get_cluster()); 
		...
		r = new op_shutdown(this->_connection, singleton<novadbi>::instance().get_cluster());
	} else {
		r = new op_error(this->_connection); 
	}
